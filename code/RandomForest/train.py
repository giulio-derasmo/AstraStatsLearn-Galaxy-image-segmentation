...
def train_model(X, y, classifier):
    ...
    if classifier == "SVM":
        ...
        model = RandomForestClassifier(n_estimators=250, max_depth=12, random_state=42, verbose=True)
        model.fit(X, y)
    elif classifier == "GBC":
        from sklearn.ensemble import GradientBoostingClassifier
        model = GradientBoostingClassifier(n_estimators=100, learning_rate=1.0, max_depth=1, random_state=42, verbose=True)
        model.fit(X, y)
